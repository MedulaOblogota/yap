...
T assert_cast(F* f_)
{
  assert(dynamic_cast<T>(f_));

  return static_cast<T>(f_);
}
...
public:
    ...
    PublicConvoy(const PublicConvoy &other) :
      ...
      m_pt1(new Minibus(*assert_cast<Minibus*>(other.m_pt1))),
      m_pt2(new Taxi(*assert_cast<Taxi*>(other.m_pt2))),
      m_m(other.m_m),
      m_t(other.m_t)
    {
    }
...
private:
    ...
    // PublicConvoy PublicConvoy(const PublicConvoy& pc);
    // PublicConvoy(const PublicConvoy &);
};
